...
const StartCallButton = memo(({ nearbyDoctors, navigation }) => {
  ...
  // Separate useEffect for glow animation
  useEffect(() => {
    ...
    // Only start glow animation if there are live doctors
  ...
  }, [liveDoctors?.length]); // Only restart when liveDoctors count changes
