...
const CustomInput = ({ title, value, onChangeText, keyboardType = 'default', autoCapitalize = 'sentences', isPassword = false, icon, error }) => {
  const [showPassword, setShowPassword] = useState(false);
  const [isFocused, setIsFocused] = useState(false);
  ...
  return (
    <View style={styles.inputContainer}>
      <Text style={styles.inputLabel}>{title}</Text>
      <View style={[styles.inputWrapper, isFocused && styles.inputWrapperFocused, error && styles.inputError]}>
        {icon && <Text style={styles.inputIcon}>{icon}</Text>}
        <TextInput
          style={styles.textInput}
          value={value}
...
const CustomButton = ({ title, onPress, loading = false, variant = 'primary', disabled = false }) => (
  <TouchableOpacity 
    style={[styles.button, variant === 'secondary' && styles.secondaryButton, disabled && styles.buttonDisabled]} 
    onPress={onPress}
...
const LoginScreen = () => {
  ...
  useEffect(() => {
    const configureGoogleSignIn = async () => {
      try {
        ...
        console.error('Google Sign-In configuration failed:', error);
        setTimeout(() => GoogleSignin.configure(GOOGLE_SIGN_IN_CONFIG), 1000);
      }
    };
    configureGoogleSignIn();
  }, []);

  // Sign out any existing Google session on mount
  useEffect(() => {
    const clearGoogleSession = async () => {
      try {
        ...
        console.log('No active Google session to clear');
      }
    };
    clearGoogleSession();
  }, []);
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // Sign out from any previous session first
      try {
        const isSignedIn = await GoogleSignin.isSignedIn();
        if (isSignedIn) {
          await GoogleSignin.signOut();
        ...
        console.log('Sign out not needed');
      }
      ...
      if (!signInResponse || signInResponse.type !== 'success') {
      ...
      if (signInResponse.type === 'success') {
        ...
        if (isLogin) {
          ...
          try {
            ...
            // Try alternative with user.id
            if (error.response?.status === 401) {
              try {
                const altLoginRes = await apiClient.post(API_CONFIG.endpoints.googleLogin, {
                  email: user.email,
                  google_token: user.id,
                  role: 'pet'
                });
                
                const loginData = altLoginRes.data || {};
                const userFromApi = loginData.user || loginData.data?.user || loginData.data;
                const token = loginData.token || loginData.accessToken || loginData.data?.token;
                const chatRoomToken = loginData.chat_room?.token || loginData.sessionToken || loginData.data?.SessionToken || null;
          ...
          const registerRes = await apiClient.post(API_CONFIG.endpoints.initialRegister, registrationData);
          ...
          try {
            ...
            if (userFromApi && token) {
              ...
              // Sign out from Google after successful registration
              await GoogleSignin.signOut();
            } else {
              throw new Error('Invalid server response');
            }
      ...
      const errorMessage = handleApiError(error, 'Google Auth');
      Alert.alert("Error", errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  const validateForm = () => {
    ...
    if (!isLogin && userType === 'vet' && !formData.fullName.trim()) {
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      if (isLogin) {
        ...
        if (!userFromApi || !tokenFromApi) {
          Alert.alert("Login failed", "Could not retrieve token from server.");
          return;
        }
        ...
        const profileKey = userFromApi?.id ? `profileCompleted:${userFromApi.id}` : null;
        await AsyncStorage.setItem(profileKey, 'true');
        updateUser({ ...userFromApi, role: userType === 'vet' ? 'vet' : 'pet', profileCompleted: true });
        Alert.alert("Success", "Login successful!");
        navigation.navigate('HomePage');
      } else {
        // Registration for vet
        const locationData = await requestLocationPermission();
        ...
        if (registerRes.data.status === "error") throw new Error(registerRes.data.message);
      ...
      const errorMessage = handleApiError(error, isLogin ? 'login' : 'registration');
      Alert.alert("Error", errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      <ImageBackground 
        ...
        source={{ uri: 'https://images.unsplash.com/photo-1450778869180-41d0601e046e?q=80&w=1920&auto=format&fit=crop' }}
        resizeMode="cover"
      >
        ...
        >
          ...
          >
            ...
            <View style={styles.contentCard}>
              <Text style={styles.titleText}>{isLogin ? 'Welcome Back' : 'Get Started'}</Text>
              <Text style={styles.subtitleText}>
                {isLogin ? 'Please login to continue' : 'Create your account'}
              </Text>

              <View style={styles.userTypeContainer}>
                <Text style={styles.sectionTitle}>{isLogin ? 'Login' : 'Register'} as:</Text>
                <View style={styles.toggleContainer}>
                  <TouchableOpacity
                    style={[styles.toggleOption, userType === 'pet_owner' && styles.activeToggle]}
                    onPress={() => setUserType('pet_owner')}
                  >
                    <Text style={styles.toggleIcon}>🐶</Text>
                    <Text style={[styles.toggleText, userType === 'pet_owner' ? styles.activeToggleText : styles.inactiveToggleText]}>
                      Pet Owner
                    </Text>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.toggleOption, userType === 'vet' && styles.activeToggle]}
                    onPress={() => setUserType('vet')}
                  >
                    <Text style={styles.toggleIcon}>👨‍⚕️</Text>
                    <Text style={[styles.toggleText, userType === 'vet' ? styles.activeToggleText : styles.inactiveToggleText]}>
                      Veterinarian
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                {userType === 'pet_owner' && (
                  <>
                    <View style={styles.googleSection}>
                      ...
                      >
                        {loading ? (
                          <ActivityIndicator color={colors.primary} size={moderateScale(20)} />
                        ) : (
                          <>
                            <Image 
                              source={{ uri: 'https://developers.google.com/identity/images/g-logo.png' }} 
